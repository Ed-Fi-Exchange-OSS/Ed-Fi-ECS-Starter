...
psql --no-password --host "$POSTGRES_HOSTNAME"  --username "$POSTGRES_USER" --port $POSTGRES_PORT --dbname "EdFi_Security" --file /tmp/EdFi_Security.sql 1> /dev/null

echo "Loading Admin database from backup..."
psql --no-password --host "$POSTGRES_HOSTNAME"  --username "$POSTGRES_USER" --port $POSTGRES_PORT --dbname "EdFi_Admin" --file /tmp/EdFi_Admin.sql 1> /dev/null
